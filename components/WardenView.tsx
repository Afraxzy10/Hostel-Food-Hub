...
import { getPreferencesForDate, setCutoffStatus, areSubmissionsOpen } from '../services/preferenceService';
import { Preference, PreferenceChoice, Department, Year, DailyCounts, AggregatedCounts, DepartmentCounts } from '../types';
...
const preferenceDisplayMap: Record<Preferimport React, { useState, useEffect, useMemo } from 'react';
// Fix: Use Firebase v8 compat syntax; remove v9 modular imports
import { db } from '../services/firebase';
import { setCutoffStatus } from '../services/preferenceService';
import { Preference, PreferenceChoice, Department, Year, DailyCounts, AggregatedCounts, DepartmentCounts } from '../types';
import { DEPARTMENTS, YEARS } from '../constants';
import { ChevronDownIcon } from './icons';
...
const DepartmentAccordion: React.FC<DepartmentAccordionProps> = ({ department, counts, preferences }) => {
    ...
    return (
        <div className="bg-white rounded-lg shadow-md mb-4">
            <button onClick={() => setIsOpen(!isOpen)} className="w-full flex justify-between items-center p-4 text-left font-semibold text-gray-800 hover:bg-gray-50">
                <span>{department}</span>
                 <div className="flex items-center space-x-4">
                    <span className="text-sm font-normal bg-blue-100 text-blue-800 px-2 py-1 rounded-full">Total: {counts.total.total}</span>
                        ...
                        <tbody>
                            {YEARS.map(year => (
                                <tr key={year} className="bg-white border-b hover:bg-gray-50">
                                    ...
                                    <td className="px-4 py-2 font-bold">{counts.years[year].total}</td>
                                </tr>
                            ))}
...
export const WardenView: React.FC<WardenViewProps> = ({ onLogout }) => {
  ...
  useEffect(() => {
    // Set up a real-time listener for today's preferences
    // Fix: Use Firebase v8 compat syntax
    const q = db.collection("preferences").where("date", "==", todayString);
    // Fix: Use Firebase v8 compat syntax
    const unsubscribe = q.onSnapshot((querySnapshot) => {
      const todaysPreferences: Preference[] = [];
      querySnapshot.forEach((doc) => {
        todaysPreferences.push(doc.data() as Preference);
      });
      setPreferences(todaysPreferences);
    });

    // Listener for cutoff status
    // Fix: Use Firebase v8 compat syntax
    const cutoffDocRef = db.collection('settings').doc('cutoff');
    // Fix: Use Firebase v8 compat syntax
    const unsubscribeCutoff = cutoffDocRef.onSnapshot((doc) => {
        // Fix: Use Firebase v8 compat property `exists` instead of `exists()`
        if (doc.exists) {
            setSubmissionsOpen(doc.data()!.submissionsOpen);
        } else {
            setSubmissionsOpen(true); // Default to open
        }
    });
  ...
  const handleToggleSubmissions = () => {
      ...
      setSubmissionsOpen(newStatus); // Optimistic UI update
  ...
  const { aggregatedCounts, grandTotal } = useMemo(() => {
    ...
    preferences.forEach(pref => {
        ...
        if (counts[department] && counts[department].years[year]) {
            ...
            switch (pref.choice) {
              case PreferenceChoice.EGG: yearCounts.egg++; deptTotal.egg++; total.egg++; break;
              case PreferenceChoice.VEG: yearCounts.veg++; deptTotal.veg++; total.veg++; break;
              case PreferenceChoice.NON_VEG: yearCounts.nonVeg++; deptTotal.nonVeg++; total.nonVeg++; break;
              case PreferenceChoice.SKIP_EGG:
              case PreferenceChoice.SKIP_MEAL: yearCounts.skipped++; deptTotal.skipped++; total.skipped++; break;
            }
        }
    });
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-4 md:p-8">
      <div className="flex flex-wrap justify-between items-center mb-6 gap-4">
        <div>
          <h1 className="text-3xl md:text-4xl font-bold text-gray-800">Warden Dashboard</h1>
          <p className="text-gray-500 mt-1">{todayDisplay}</p>
        </div>
        <button onClick={onLogout} className="bg-gray-200 text-gray-700 font-semibold py-2 px-4 rounded-lg hover:bg-gray-300 transition-colors">Logout</button>
      </div>
      ...
      <div className="bg-blue-50 p-4 rounded-lg shadow-inner mb-8">
        <h2 className="text-xl font-bold text-blue-800 mb-2">Grand Totals for Today</h2>
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-4 text-center">
            <div className="p-2 bg-white rounded shadow-sm"><p className="font-bold text-2xl">{grandTotal.total}</p><p className="text-sm text-gray-600">Total Submissions</p></div>
            <div className="p-2 bg-white rounded shadow-sm"><p className="font-bold text-2xl">{grandTotal.egg}</p><p className="text-sm text-gray-600">Eggs</p></div>
            <div className="p-2 bg-white rounded shadow-sm"><p className="font-bold text-2xl">{grandTotal.veg}</p><p className="text-sm text-gray-600">Veg</p></div>
            <div className="p-2 bg-white rounded shadow-sm"><p className="font-bold text-2xl">{grandTotal.nonVeg}</p><p className="text-sm text-gray-600">Non-Veg</p></div>
            <div className="p-2 bg-white rounded shadow-sm"><p className="font-bold text-2xl">{grandTotal.skipped}</p><p className="text-sm text-gray-600">Skipped</p></div>
        </div>
      </div>
...
export const WardenView: React.FC<WardenViewProps> = ({ onLogout }) => {
  const [preferences, setPreferences] = useState<Preference[]>([]);
  const [submissionsOpen, setSubmissionsOpen] = useState(areSubmissionsOpen());
  ...
  useEffect(() => {
    setPreferences(getPreferencesForDate(todayString));
  }, [todayString]);
